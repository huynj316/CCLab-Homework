 ...
 * Example 2:
 * Graphics Example
 * Adding size changing aniation to the ball
 * Adding transparency and color changing effect to the ball
 */
...
void ofApp::setup(){
    ...
    ofSetWindowTitle("Moving Stuffs");
}
...
void ofApp::draw(){
    ...
    for(unsigned int i = 1; i < points.size(); i++){
        ...
        float thickness = ofMap(distance, 0, 100, 200, 2, true);
        
        //calculate the points to the left and to the right
        //by extending the current point in the direction of left/right by the length
        float colorVar = colorVar + 0.4f;
        ofSetColor(180,120,(int)(colorVar * 5.0f) % 255,(int)(colorVar * 10.0f) % 255);
